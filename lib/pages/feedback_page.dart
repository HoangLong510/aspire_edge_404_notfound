...
class _FeedbackPageState extends State<FeedbackPage> {
  ...
  // NEW: lọc theo số sao (null = tất cả)
  int? _ratingFilter; // 1..5 hoặc null

  // Trạng thái feedback của chính user
              ...
              child: Row(
                children: [
                  ...
                  if (!_isAdmin) ...[
                    if (!_hasMyFeedback)
                      ElevatedButton.icon(
                        onPressed: () => Navigator.of(context).pushNamed('/feedback_form'),
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snap) {
                  ...
                  // Toàn bộ docs từ query (có thể đã filter unanswered trên server)
                  final allDocs = (snap.data?.docs ?? []).toList();

                  // Đếm số lượng theo sao dựa trên allDocs (để hiển thị bộ đếm)
                  final starCounts = List<int>.filled(6, 0); // 0..5
                  ...
                  // Áp dụng lọc theo số sao trên client
                  ...
                  // Sort mặc định: theo CreatedAt (mới -> cũ)
                  shownDocs.sort((a, b) {
                  ...
                  // UI thanh FILTER THEO SAO (cuộn ngang để tránh overflow)
                                      ...
                                      children: [
                                        ...List.generate(5, (idx) {
                                          ...
                                          return Icon(
                                            ...
                                            size: 18,
                                            color:
                                                filled ? Colors.amber[700] : Colors.grey[500],
                                          );
                                        }),
                                        if (emoji.isNotEmpty) ...[
                                          const SizedBox(width: 6),
...
class _UserAvatar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Nếu feedback ẩn danh thì icon ẩn danh
